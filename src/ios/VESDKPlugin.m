...
+ (void)setWillPresentVideoEditViewController:
    (CDV_VESDKWillPresentBlock)willPresentBlock {
  _willPresentVideoEditViewController = willPresentBlock;
}

const struct VESDK_IMGLY_Constants VESDK_IMGLY = {
...
- (void)present:(CDVInvokedUrlCommand *)command {
  ...
  if (self.lastCommand == nil) {
    ...
    IMGLYMediaEditViewControllerBlock createMediaEditViewController =
      ...
      CDV_VESDKWillPresentBlock willPresentVideoEditViewController =
          VESDKPlugin.willPresentVideoEditViewController;
    ...
    IMGLYUTIBlock getUTI =
      ...
      return configuration.videoEditViewControllerOptions
          .videoContainerFormatUTI;
    };
    ...
    NSURL *exportFile =
        [VESDKConvert VESDK_IMGLY_ExportFileURL:valueExportFile
    ...
    NSURL *serializationFile =
        [VESDKConvert VESDK_IMGLY_ExportFileURL:valueSerializationFile
    ...
    if ((exportType == nil) ||
        (exportFile == nil &&
         [exportType isEqualToString:VESDK_IMGLY.kExportTypeFileURL]) ||
        (serializationFile == nil &&
         [serializationType isEqualToString:VESDK_IMGLY.kExportTypeFileURL])) {
      CDVPluginResult *result =
          [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
    ...
    [[NSFileManager defaultManager]
               createDirectoryAtURL:exportFile.URLByDeletingLastPathComponent
    ...
    configuration = [[PESDKConfiguration alloc]
        initWithBuilder:^(PESDKConfigurationBuilder *_Nonnull builder) {
          ...
          IMGLYConfigurationBlock configureWithBuilder =
              VESDKPlugin.configureWithBuilder;
    ...
    if (error != nil) {
      NSString *message = [NSString
          stringWithFormat:@"Error while updating configuration: %@", error];
      CDVPluginResult *result =
          [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
                            messageAsString:message];
      [self closeControllerWithResult:result];
...
- (void)videoEditViewController:
  ...
  if (self.serializationEnabled) {
    ...
    if ([self.serializationType
            isEqualToString:VESDK_IMGLY.kExportTypeFileURL]) {
      ...
      if ([serializationData VESDK_IMGLY_writeToURL:self.serializationFile
        ...
        serialization = self.serializationFile.absoluteString;
                   ...
                   isEqualToString:VESDK_IMGLY.kExportTypeObject]) {
  ...
  [self dismiss:videoEditViewController
        animated:YES
      completion:^{
        if (error == nil) {
          ...
          NSDictionary *payload = [NSDictionary
              dictionaryWithObjectsAndKeys:(url != nil) ? url.absoluteString
                                                        : [NSNull null],
                                           @"video",
...
- (void)videoEditViewControllerDidCancel:
    (PESDKVideoEditViewController *)videoEditViewController {
  CDVPluginResult *result =
...
- (void)videoEditViewControllerDidFailToGenerateVideo:
    (PESDKVideoEditViewController *)videoEditViewController {
  CDVPluginResult *result =
      [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
